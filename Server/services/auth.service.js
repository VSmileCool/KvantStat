...
class UserService{
    async registration(name, code, email, password){
        const candidate = await user_model.findOne({where:{user_email: email}})
            if (candidate){
            ...
            const user = await user_model.findOne({where:{user_email: email}})
    ...
    async refresh(refreshToken) {
        ...
        const user = await user_model.findByPk(userData.id);
        const userDto = new UserDto(user.user_name, user.user_id);
        const tokens = tokenService.generateTokens({...userDto});
